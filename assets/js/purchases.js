		 ...
		 var form=$("#feedback_rating_form");
			...
			if(chat_content.length > 0 ){ 
			 ...
			 var url  =  base_url+'user/purchases/save_feedback';
				...
				$("#_error_msg").html('<div class="account-error">'+please_add_rating+'</div>');

			}

			}else{ 

				$("#_error_msg").html('<div class="account-error">'+please_enter_some_content+'</div>');
			...
			return false;
			 ...
			 $("#_error_msg").html('<div class="account-error">'+please_select_users+'</div>');
 	  ...
 	  $.ajax({
			  ...
			  success: function(res){ 
			  	...
			  	if(source == 'paypal'){
			  		...
			  		$('input[value="paytabs"]').attr('checked','checked');
			...
			if(source == 'stripe'){
				...
				$('#check_and_fillaccount').attr('onclick', "checkandfillaccount('stripe')");
			...
			if(source == 'paypal'){
				...
				$('#check_and_fillaccount').attr('onclick', "checkandfillaccount('paypal')");
...

{
	 ...
	 if(payment_soruce=='paytabs')
	 {
	 	var dataString="p_id="+p_id+"&sts="+reason+"&payment_soruce="+payment_soruce; 
					  $.ajax({
							  url:url,
							  data:dataString,
							  type:"POST",
	 ...
	 if( (reason.length >0) &&((paypal_email.length >0) || (account_holder_name.length >0 && account_number.length >0 && bank_name.length >0 && bank_address.length >0) &&(sort_code.length >0 || routing_number.length >0 || account_ifsc.length >0)) )
	 {
		...
		if(payment_soruce == 'paypal'){
			...
			$("#reason_errormsg").text(please_enter_cancellation_reason_and_paypal_id);	
			}else if(reason.length <= 0){
				$("#reason_errormsg").text(please_enter_cancellation_reason);	
			}else if(paypal_email.length <=0 ){
					$("#reason_errormsg").text(please_enter_the_paypal_id);	
			}	
...

{
	 ...
	 if(val == 'paypal'){
		...
		$("#paypal_errormsg").text(please_enter_id);
		...
		if(sort_code.val()=="" && routing_number.val()=="" && account_ifsc.val()==""){
			...
			$('.error_note').text(please_provide_your_bank_sort_code_or_routing_number_or_ifsc_code);
		...
		$("#paypal_errormsg").html(please_enter_id);
...
{	
	if(type == 'paytabs'){
	 	...
	 	$('.decline_stripe_input_content').hide();	

 	 }
	 else if(type == 'stripe'){

	 	$(".payment_type_block").hide();
	 	$('.decline_stripe_input_content').show();	
...

{
	...
	var url    =  base_url+'user/purchases/accept_seller_request';
	 ...
	 var payment_soruce= $("#buyer_accept_rowtype").val();
	...
	var paypal_email =  $("#paypal_emailid").val(); 
	 ...
	 var account_holder_name   =  $("#decline_account_holder_name").val(); 	 
	 var account_number   =  $("#decline_account_number").val(); 	 
	 var bank_name   =  $("#decline_bank_name").val(); 	 
	 var bank_address   =  $("#decline_bank_address").val(); 	 
	 var sort_code   =  $("#decline_sort_code").val(); 	 
	 var routing_number   =  $("#decline_routing_number").val(); 	 
	 var account_ifsc   =  $("#decline_account_ifsc").val();
	 ...
	 if(payment_soruce=='paytabs')
	 {
	 		 var dataString="p_id="+id+"&payment_soruce="+payment_soruce;
	 		 $.ajax({
	 ...
	 if(((paypal_email.length >0) || (account_holder_name.length >0 && account_number.length >0 && bank_name.length >0 && bank_address.length >0) &&(sort_code.length >0 || routing_number.length >0 || account_ifsc.length >0)) )
	 {
		  var dataString="p_id="+id+"&pemail="+paypal_email+"&account_holder_name="+account_holder_name+"&account_number="+account_number+"&bank_name="+bank_name+"&bank_address="+bank_address+"&sort_code="+sort_code+"&routing_number="+routing_number+"&account_ifsc="+account_ifsc+"&payment_soruce="+payment_soruce;
		  ...
		  $.ajax({
				 ...
				 if(payment_soruce == 'paypal'){
					 if(paypal_email.length <=0 ){
							$("#reason_errormsgone").text(please_enter_the_paypal_id);	
					}	
				}else if(payment_soruce == 'stripe'){

					
					if($('#decline_account_holder_name').val()==""){
						$('#decline_account_holder_name').css({"border": "red 2px solid"});
					}else{
						$('#decline_account_holder_name').removeAttr('border');
					}
					if($('#decline_account_number').val()==""){
						$('#decline_account_number').css({"border": "red 2px solid"});
					}else{
						$('#decline_account_number').removeAttr('border');
					}
					if($('#decline_bank_name').val()==""){
						$('#decline_bank_name').css({"border": "red 2px solid"});
					}else{
						$('#decline_bank_name').removeAttr('border');
					}
					if($('#decline_bank_address').val()==""){
						$('#decline_bank_address').css({"border": "red 2px solid"});
					}else{
						$('#decline_bank_address').removeAttr('border');
					}
					if(($('#decline_sort_code').val()=="") && ($('#decline_routing_number').val()=="") && ($('#decline_account_ifsc').val()=="")){
						$('#decline_sort_code').css({"border": "red 2px solid"});
						$('#decline_routing_number').css({"border": "red 2px solid"});
						$('#decline_account_ifsc').css({"border": "red 2px solid"});

					}else{
						$('#decline_sort_code ,#decline_routing_number, #decline_account_ifsc').removeAttr('border');
					}	
