		 ...
		 $.ajax({
				  ...
				  success: function(res){ 
 					    $.each(res.new_chat, function(key,value){
							 if($("#chat_details_appnd").html() == '<p class="text-red"> &nbsp; &nbsp; '+no_chats_available+' </p>') {
	  ...
	  $.ajax({
		  ...
		  success: function(res){
			  if(res==1)
			  {
				  ...
				  $("#chat_list_id"+user_id).html('<span class="time-text pull-right"> </span><span class="clear text-ellipsis text-xs msg-text">'+no_message+'</span>');
				 // location.reload();
			  }
		  }
	  });
    ...
    if(chat_id){ 
	 ...
	 if(temp_chat ==0){
		 ...
		 var url  =  base_url+'user/chat/save_chat';
		 	...
		 	$("#_error_").html('<div class="account-error">'+please_enter_some_content+'</div>');
			...
			success: function(res) { 
			    if(res.sts == 0){
				    ...
				    $('#new_status_attaches').append(res.content);
					 ...
					 $(".msg-progress").hide();
					 $("#upload_error_image").text(error_occured+','+please_upload_image_file_only);
				}
  			}
		});
   }
			...
			success: function(res) { 
			    if(res.sts == 0){
				    ...
				    $('#new_status_attachesfile').append(res.content);
					 ...
					 $(".msg-progressone").hide();
					 $("#upload_error_image").html(error_occured+','+please_upload_image_file_only);
			 ...
			 if (pos=='top' && autoload_total !='') {
				...
				if(chat_track_load < autoload_total && chat_loading==false) //there's more data to load
				{
				 ...
				 $.ajax({
						  ...
						  beforeSend: function(){
							  chat_loading = true;
							  $('#chat_details_appnd').prepend('<div class="text-center chat-date clm-lc-span"><span>'+loading_conversation+'..!</span></div>'); 
						  },
						  success: function(res){
							  $('.clm-lc-span').remove();
