...
.prctitle.platinum{ background:#9576dc;}
 ...
 $i = 1;
    ...
    <?php   $this->session->unset_userdata('message'); } ?>
            <div class="row">
                ...
                $check_already_free_subscripe =$this->db->get_where('subscriptions_payments_logs',$where)->result_array();

                
                if(!empty($check_already_free_subscripe))
                  {
                    $already_subscriptionprice=0;
                  }



                  $check_free_subscripe =$this->db->get_where('subscriptions_payments',$where)->row_array();
                  if(!empty($check_free_subscripe))
                  {
                    $subscriptionprice=$check_free_subscripe['subscription_amount'];
                  }
            	 ...
            	 <div class="col-md-4">
                	<div class="prccontainer">
                        <div class="prctitle platinum">
                            ...
                            <?php 
                            if($subscriptionprice==$rows['subscription_rate'] || $already_subscriptionprice==$rows['subscription_rate'])
                            {
                            ?>
                                   <a href="#" style="color:pink;background: #fff;cursor: auto;">Already Subscribed</a>
                              <?php
                            }
                            else
                            {
                              ...
                              ?>
                              <a href="#" data-toggle="modal" data-target="#checkout-popup" onclick="sub_scriptions('<?php echo $rows['id'];?>','<?php echo $rows['subscription_name'];?>','<?php echo $rows['subscription_rate'];?>','<?php echo $rows['subscription_period'].' '.$rows['period_type'];?>','<?php echo $rows['no_of_gigs'];?>')">Get it now <?php if($rows['subscription_rate']==0){echo '(Free)';};?></a>
                              <?php
                              
                           }
                            ?>                          
                            
                            <div class="gigscount">Number of Gigs: <?php echo $rows['no_of_gigs'];?></div>
                        </div>                        
                    </div>
                </div>
            ...
            <?php } }
                ...
                ?>
                <h3 class="header-title">No Subscriptions Found</h3>
           <?php } ?>
            </div>
        </div>
    </div>
                ...
                <div class="modal-dialog">
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-header text-center">
                            ...
                            <h4 class="sign-title">Subscription</h4>
