...
class Installer extends CI_Controller
{
    ...
    public function purcahse_code()
    {
        	if ($_POST['purcahse_code']) {
					...
					$array_inputs = array('purcahse_code' => $value,'site_name'=>$site_name,'site_ip'=>$site_ip);
			        $ch = curl_init(); 
	...
	public function create_tables()
	{
        ...
        public function admin_details()
        {
			...
			$mysqli = new mysqli($hostname, $db_username, $db_password, $database_name); 
                ...
                if(is_file(getcwd()."/assets/temp_files/install_4.sql"))
                {

                    $query = file_get_contents('assets/temp_files/install_4.sql');
                    $mysqli->multi_query(utf8_encode($query));
        ...
        {        	    
        		 ...
        		 $site_theme= $this->input->post('site_theme');
        		 	...
        		 	if($site_theme=='Default Theme')
        		 	{
        		 		$file_name='default_theme.zip';
        		 	}
        		 	elseif($site_theme=='Theme One')
        		 	{
        		 		$file_name='theme_one.zip';
        		 	}
				...
				if( !$copy ) {
				    ...
				    $this->session->set_userdata(array(
								'error_message' => " Failed to theme upload "));	
				                redirect(base_url() .'installer/?step=4');
				}
				else{
                           ...
                           $zip = new ZipArchive;
							...
							if ($zip->open($file) === TRUE) {
								...
								$system_settings['site_theme']       = $this->input->post('site_theme');
				                ...
				                $system_settings['gigs_payment_option']        = $this->input->post('gigs_payment_option');
