...
class Gigs_model extends CI_Model{
    ...
    {       
        ...
        $query = $this->db->query("SELECT sell_gigs.category_id,categories.name FROM `sell_gigs` LEFT JOIN categories ON categories.CATID = sell_gigs.category_id WHERE sell_gigs.status = 0 AND name IS NOT null GROUP BY sell_gigs.category_id ORDER BY sell_gigs.total_views DESC LIMIT 0 , 4");
